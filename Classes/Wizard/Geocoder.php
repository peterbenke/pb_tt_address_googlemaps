<?php
namespace PeterBenke\PbTtAddressGooglemaps\Wizard;
 ...
 * @package PeterBenke\PbTtAddressGooglemaps\Wizard
 */
...
class Geocoder extends \TYPO3\CMS\Backend\Controller\Wizard\AbstractWizardController {
	...
	public function __construct() {
		parent::__construct();
		$this->getLanguageService()->includeLLFile('EXT:pb_tt_address_googlemaps/Resources/Private/Language/locallang_be.xlf');
		$this->init();
	}
	...
	protected function init(){
		...
		$this->doc->addStyleSheet('Google Geocoder','../typo3conf/ext/pb_tt_address_googlemaps/Resources/Public/Css/geocoder.css');
	...
	/**
	 * Get the postDecimals from field tt_address.latitude
	 * @return int
	 */
	private function getPostDecimalsFromLatLon(){
		 ...
		 */
		$db = $GLOBALS['TYPO3_DB'];
		$query = "SELECT COLUMN_TYPE FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA = '" . $GLOBALS['TYPO3_CONF_VARS']['DB']['database'] . "' AND TABLE_NAME = 'tt_address' AND COLUMN_NAME = 'latitude';";
		$result = $db->admin_query($query);

		$columnType = $result->fetch_row()[0]; // => e.g. decimal(14,12)

		// Readable: ;-)
		//
		// [A-Za-z]+   \(   [0-9]+ , (.*)     \)
		// decimal      (   14     ,  12       )
		$postDecimals = intval(preg_replace('/^[A-Za-z]+\([0-9]+,(.*)\)$/', '$1', $columnType));

		if(!($postDecimals > 0)){
			$postDecimals = $fallback;
		}
	...
	protected function getInlineJs($addressData){
		...
		$postDecimalsLatLon = $this->getPostDecimalsFromLatLon();
		...
		*/
		...
		$js.='
			...
			if (parent.window.opener){
				...
				var addressAddress = parent.window.opener.TYPO3.jQuery("[data-formengine-input-name=\'data[tt_address][' . $addressData['uid'] . '][address]\']").val();
				var addressZip = parent.window.opener.TYPO3.jQuery("[data-formengine-input-name=\'data[tt_address][' . $addressData['uid'] . '][zip]\']").val();
				var addressCity = parent.window.opener.TYPO3.jQuery("[data-formengine-input-name=\'data[tt_address][' . $addressData['uid'] . '][city]\']").val();
				// Selectbox => static_countries
				var addressCountry = parent.window.opener.TYPO3.jQuery("select[name=\'data[tt_address][' . $addressData['uid'] . '][country]\'] option:selected").text();
				// Try to get the value from input-field (static_info_tables is not installed)
				if(addressCountry == ""){
					addressCountry = parent.window.opener.TYPO3.jQuery("[data-formengine-input-name=\'data[tt_address][' . $addressData['uid'] . '][country]\']").val();
				}
		...
		$js.= '
			...
			function findAddress(address){
				...
				geocoder.geocode(address, function(results, status) {
					...
					if (status == google.maps.GeocoderStatus.OK) {
						 ...
						 */
						TYPO3.jQuery("#latitude").val(results[0].geometry.location.lat());
						TYPO3.jQuery("#longitude").val(results[0].geometry.location.lng());
			...
			function insertValuesInParentWindow(){
				if (parent.window.opener)	{
					// Visible fields
					parent.window.opener.TYPO3.jQuery("input[data-formengine-input-name=\'data[tt_address][' . $addressData['uid'] . '][latitude]\']").val(roundDataToNumber(TYPO3.jQuery("#latitude").val(),' . $postDecimalsLatLon . '));
					parent.window.opener.TYPO3.jQuery("input[data-formengine-input-name=\'data[tt_address][' . $addressData['uid'] . '][longitude]\']").val(roundDataToNumber(TYPO3.jQuery("#longitude").val(),' . $postDecimalsLatLon . '));
					// Hidden fields
					parent.window.opener.TYPO3.jQuery("input[name=\'data[tt_address][' . $addressData['uid'] . '][latitude]\']").val(roundDataToNumber(TYPO3.jQuery("#latitude").val(),' . $postDecimalsLatLon . '));
					parent.window.opener.TYPO3.jQuery("input[name=\'data[tt_address][' . $addressData['uid'] . '][longitude]\']").val(roundDataToNumber(TYPO3.jQuery("#longitude").val(),' . $postDecimalsLatLon . '));
					window.close();
				} else {
					showErrorMessage("' . $this->getLanguageService()->getLL('geocoding.error.referenceToParentWindow') . '");
				}
	...
	protected function main(){
		...
		$this->content .='
			...
			<form id="transferGeoData" name="transferGeoData" class="form-inline">
			  ...
			  <div class="form-group">
				<label for="latitude">' . $this->getLanguageService()->getLL('geocoding.form.transferGeoData.label.latitude') . ':</label>
				<input type="text" class="form-control" id="latitude" readonly>
			  </div>			
			
			  <div class="form-group">
				<label for="longitude">' . $this->getLanguageService()->getLL('geocoding.form.transferGeoData.label.longitude') . ':</label>
